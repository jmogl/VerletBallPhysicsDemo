...
/*
	...
	- Replaced spaces with tabs for code indentations to clean up the formatting when displayed at GitHub
...
//		getOrientation();
	...
	var Simulation = function(context){
		...
		// Define line for bottom border. Not using interior wall for maximum stability since they have addedd impulse
		...
		var collide = function(preserve_impulse){
			...
			for(var i=0, l=bodies.length; i<l; i++){
				...
				for(var j=i+1; j<l; j++){
					...
					// if the spheres are closer then their radii combined:
					if(length < target){
